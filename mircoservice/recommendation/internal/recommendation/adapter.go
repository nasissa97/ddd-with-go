...
func NewPartnershipAdaptor(
	client *http.Client,
	url string,
) (*PartnershipAdaptor, error) {
	if client == nil {
...
func (p PartnershipAdaptor) GetAvailability(
	ctx context.Context,
	tripStart time.Time,
	tripEnd time.Time,
	location string,
	...
	if err != nil {
		return nil, fmt.Errorf("faild to call partnerships: %v", err)
	}
	...
	if err := json.NewDecoder(res.Body).Decode(&pr); err != nil {
		return nil, fmt.Errorf("could not decode the response body of partnerships: %v", err)
	}
